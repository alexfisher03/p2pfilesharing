...
class NetworkManager:
    ## constructor for our networking manager : opening, closing sockets.
    def __init__(self, self_peer_id, self_port, on_connected, on_message, on_disconnected):
        self.self_peer_id = self_peer_id        # peer id 
        self.self_port = self_port              # port number
    ...
    ### handle incoming connection - handles handshake 
    ...
    def _handle_incoming(self, sock):
        try:
            ...
            print(f"handshake failed on incoming connection: {e}")
            sock.close()
            return
        lock = threading.Lock()
    ...
    # send a message to all connected peers, optionally skipping some in exclude 
    ...
    def _disconnect(self, remote_peer_id):
        if remote_peer_id not in self.connections:
            return
        sock, _ = self.connections.pop(remote_peer_id) ## pop from our dictionary
        try:
    ...
    # close everything
    def shutdown(self): ### iterate through and call our disconnect function
        self._shutdown.set()
        for peer_id in list(self.connections):
